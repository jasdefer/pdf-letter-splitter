...
def main():
    """Example usage of the letter analysis module."""
    import json
    import sys
    from pathlib import Path
    
    # Try to import extract_text module
    try:
        from extract_text import extract_text_from_pdf
    except ImportError:
        print("Error: extract_text module not found", file=sys.stderr)
        print("Make sure extract_text.py is in the same directory", file=sys.stderr)
        sys.exit(1)
    
    if len(sys.argv) < 2:
        print("Usage: analyze_letters.py <input.pdf>", file=sys.stderr)
        sys.exit(1)
    ...
    # Get just the text from each page
    ocr_pages = [page['text'] for page in result['pages']]
    
    # Analyze documents
    print("Analyzing documents...", file=sys.stderr)
    ...
    print(f"\n\nFound {len(letters)} letter(s):", file=sys.stderr)
    for i, letter in enumerate(letters, 1):
        print(f"\nLetter {i}:", file=sys.stderr)
        print(f"  Start page: {letter['start_page']}", file=sys.stderr)
        print(f"  Pages: {letter['page_count']}", file=sys.stderr)
        print(f"  Date: {letter['date']}", file=sys.stderr)
        print(f"  Sender: {letter['sender']}", file=sys.stderr)
        print(f"  Topic: {letter['topic']}", file=sys.stderr)
